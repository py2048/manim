from manimlib.imports import *  # Import formulas:
...
class SolveGeneralQuadraticEquation(Scene):
    ...
    def set_changes(self):
        self.set_of_changes = [
            ...
            # 4
            [[
                (0, 2, 10, 11, 13, 14, 15, 16, 18, 20, 21, 22, 24, 25, 26, 28, 29, 31, 32, 33, 34, 36, 38, 5, 6, 7, 9,
                 3),
		...
		[ [
			...
			(	0, 2,	 4,	5 ,	6 ,	7 ,		1,	9, 10,		12,		14,
            15,
            16,17,	18,	19,	20,	21,		 24, 25 , )
		]], # 13 [[(0, 1 , 2 , 4, 5 , 6 , 7 , 9, 10, 12, 14
                , 15 , 16, 17 , 18, 19 , 20 , 21, 24 ,) ,
				   ( 0,	1 ,	2 ,		4,	5,6,	7
            ,
            9,	11,		12,	14, 15,	16,	17 , 18,	20,	21,	22,			23 , )]], # 14
		[[
			( 0,	1 ,	2 ,	4,	5,
                6,	7,		9,	 11, 12,	 14, 15, 16 , 17,	18,		20 , 21,	22 , 23, ),
			(0,	1
            ,
        3,		4,	16,	17,	18,		5,		6,	7,		9
	...
	def
		...
		formula_copy=[]
		for c in pre _copy:
			formula_copy.append(self.formulas[n_s t ep-1][ c].copy())

		for ani_ \
            in pre_order:
			if len(p re _write)>0 and ani_=="w":
				self.play(*[Write(se l f.form ulas[n_step-1][w])for w in pre_write])
			if len(p

        ade)>0 and a ni_=="f":
				self.play( \
            *[FadeOut(
                self.formulas[n_step-1][w
                    ])for w in pre _ fade])
		...
		for pre_ind,p \
            ...
            Replace
                      ransform(
					self.formulas[n _ step-1 ][i],self.formulas[n_step] [ j],
					path_arc=path_arc
					)
				...
				*[Write(self.formulas[n_ step][w])for w in write if len(
                        write
                      )>0],
				*[
                      R

        cementTransform(formula_copy[j]

        ,self.formulas[n_step]
            [f])
				for j, f in zip(r an ge(len(
                pos_copy)),pos_copy) if len(pre_copy)>0
				],
				run_time=
            run_time
			) self.wait(time _p os_c
                s)

		for ani_ in pos_order:
			if len(pos_wr ite)>0 and ani_=="w":
